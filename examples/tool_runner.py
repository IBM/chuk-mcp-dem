"""
Shared helper for running chuk-mcp-dem MCP tools directly from Python.
...
Usage:
    ...
    async def main():
        runner = ToolRunner()
        result = await runner.run("dem_list_sources")
        print(result)
"""
...
from chuk_mcp_dem.tools.analysis import register_analysis_tools
from chuk_mcp_dem.tools.discovery import register_discovery_tools
from chuk_mcp_dem.tools.download import register_download_tools
...
class ToolRunner:
    ...
    All 14 tools are registered and callable via run(tool_name, **kwargs).
