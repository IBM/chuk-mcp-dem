#!/usr/bin/env python3
"""
DEM MCP Server - Entry Point

This module provides the async MCP server for digital elevation model
discovery, retrieval, and terrain analysis.
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="DEM MCP Server")
    parser.add_argument(
        "mode",
        nargs="?",
        choices=["stdio", "http"],
    ...
    if args.mode == "stdio":
        print("DEM MCP Server starting in STDIO mode", file=sys.stderr)
        mcp.run(stdio=True)
    elif args.mode == "http":
        print(
            f"DEM MCP Server starting in HTTP mode on {args.host}:{args.port}",
            file=sys.stderr,
        )
        mcp.run(host=args.host, port=args.port, stdio=False)
    else:
        if os.environ.get(EnvVar.MCP_STDIO) or (not sys.stdin.isatty()):
            print("DEM MCP Server starting in STDIO mode (auto-detected)", file=sys.stderr)
            mcp.run(stdio=True)
        else:
            print(
                f"DEM MCP Server starting in HTTP mode on {args.host}:{args.port}",
